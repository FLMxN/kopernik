...
if __name__ == "__main__":
    ...
    # Test with an image
    test_image_path = IMG # Replace with your image path

predictor = DeterminedPredictor(model, processor, device)
# coord_predictor = CoordinatePredictor("D:/resnet50-finetuned")

settings = [
    {"temperature": 0.1, "use_tta": False, "min_confidence": 0.01},
    {"temperature": 0.1, "use_tta": True, "min_confidence": 0.01},
    {"temperature": 0.5, "use_tta": False, "min_confidence": 0.01},
    {"temperature": 0.5, "use_tta": True, "min_confidence": 0.01},
]

for i, setting in enumerate(settings, 1):
    print(f"{'='*60}")
    print(f"{'Volume' if i > 2 else 'Precision'}{'Scaled' if i % 2 == 0 else 'Unscaled'}")
    predictor.predict(test_image_path, **setting)